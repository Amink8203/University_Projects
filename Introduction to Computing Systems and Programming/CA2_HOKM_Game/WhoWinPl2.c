...
#define PLAYER 4
#define CARDS 11
//Win condition when player2 starts the round.
void WhoWinPl2(int *team1, int *team2, int *Pl1, int *Pl2,
  ...
  char *FirstP4sym, char *BN, char SecondCsym[3], int SecondCnum[3], int ThirdCnum[4])
{
    ...
    else if ((*FirstP1sym == *BN || *FirstP2sym == *BN || *FirstP3sym == *BN) && *FirstP2sym != *BN)
    {
        for (int i = 0; i < 3; i++)
        {
            if (SecondCsym[i] == *BN)
    ...
    {
        if (*FirstP1sym == *FirstP2sym)
            ThirdCnum[1] = *FirstP1num;
        if (*FirstP3sym == *FirstP2sym)
            ThirdCnum[2] = *FirstP3num;
        if (*FirstP4sym == *FirstP2sym)
